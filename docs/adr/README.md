...
the obfuscation roadmap. They lived at `warren-workspace/warrenguard-docs/` until
2026-08-02, which is why nothing in this repo referenced them; the workspace holds
no product content, so they belong here.
...
engine's. The other is `warren-core/docs/adr/` (0001 to 0006: GFW posture, exit IP
diversity, dataplane transport fallback, X.509 multihop dispatcher, DNS
content-blocking resolver, anonymous session credentials). A bare "ADR-0004" is
