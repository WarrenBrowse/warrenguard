...
- Each command is REGISTERED on a gate before it is spawned, and the guard's
  ...
  restoration the later write could overwrite. A command that does run counts as a
  restoration only when it exits zero: `run_sync_bounded` returns `Some` for any child
  that exited, so a denied cmdlet is reported with its exit status. The synchronous
  runner discards stdout and stderr because these commands do not consume output and
  a full pipe could block the teardown. Termination timing is not relied on:
...
entry: `pfctl::State` exposes no interface name, and the entry's local address is
not a substitute, because pf treats the interface a packet traversed and its source
address as distinct criteria (which is why Apple's guidance is to select an
interface with `IP_BOUND_IF` rather than rely on the address). One predicate
therefore decides both sides: the purge kills every entry the policy does not
...
| Windows: partial failure and cancellation roll back; restoration waits for accepted commands and reports failures | Lifecycle tests cover failed and cancelled install and uninstall. Gate tests cover queued and running commands, timeout refusal, and a child that exits nonzero. The synchronous runner is tested with a verbose child to ensure output cannot block restoration. Windows host behavior still requires the smoke test below. |
...
| macOS host behaviour, `/dev/pf` purge | Not verified here. The manual `pfctl -s states` procedure and ignored `real_pf_install_and_uninstall_cycle` test require root on a disposable or idle Mac. The test fails if its explicit opt-in is missing or the pre-flight refuses before loading the anchor. |
