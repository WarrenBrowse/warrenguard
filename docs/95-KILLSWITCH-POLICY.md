...
- Each command is REGISTERED on a gate before it is spawned, and the guard's
  ...
  that exited, so a denied cmdlet, or a program that could not be started at all
  (surfaced as an ordinary non-zero exit, 127 under `posix_spawn` and 9009 under the
  Windows loader), is reported like a timeout, with its status and stderr, for the
  operator to run by hand. Reading "the child exited" as "the state is back" would
  leave the firewall changed and say nothing. Termination timing is not relied on:
