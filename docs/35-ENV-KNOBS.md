...
| `WARREN_DG_BDP_FLOOR` | usize (bytes) | per side: 131072 (128 KiB) on a client, 1048576 (1 MiB) on an exit or relay | clamped to [16384, 64 MiB]; unparsable -> the side's own default | lower bound of the adaptive send buffer (ramp-up and tiny-BDP guard); it is what sizes the buffer on any path whose BDP times the multiple stays under it, so on a narrow last mile the floor IS the buffer | warrenguard-transport-core/src/transport_config.rs |
...
| `WARREN_PATH_HEALTH_SECS` | u64 (seconds) | 15 | 0 disables the prober; unparsable -> default | steady cadence of the paired small/large in-tunnel goodput probes | warrenguard-transport/src/path_health.rs |
...
| `WARREN_TCP_FALLBACK_PREFER` | bool | off | "1"/"true" enables, else off | dial the TLS-over-TCP carrier FIRST on every relay dial (the UDP race runs only if the carrier fails), regardless of what the process has observed; the automatic version of this arms itself after consecutive UDP sessions that establish and die within 60 s | warrenguard-transport/src/multihop.rs (dial_relay_carrier_first) and warrenguard-transport/src/udp_hostility.rs |
...
| `WARREN_EXIT_V2_DRAIN_SIGNAL` | bool | off | "1"/"true" enables, else off | send `/v2` sessions the drain advisory and the drain close at the deadline, as `/v1` sessions always get; off leaves an established `/v2` session alone until the exit process stops (a draining exit still refuses new connections either way). Turn it on only once the clients served handle the advisory: one that drops its tunnel on it and redials the drained exit loses the whole drain window | `knobs::exit_v2_drain_signal()` -> when `true`, call `ExitTerminateCtx::with_v2_drain_signal` (a `pq-hpke` build) |
